...
int ovs_f3_nla_copy_actions(const struct nlattr *attr,
			  struct sw_flow_actions **sfa)
{
	const struct nlattr *a;
	int rem, err;
			 ...
			 struct sw_flow_actions **sfa)
{
	...
	int rem, err;
		#ifdef OVS_DEBUG
	printk("ovs_nla_copy_actions enter the function!\n");
		#endif
	...
	nla_for_each_nested(a, attr, rem) {
		...
		static const u32 action_lens[OVS_ACTION_ATTR_MAX + 1] = {
			...
			[OVS_ACTION_ATTR_PUSH_F3_HEADER] = (u32)-1,
		        [OVS_ACTION_ATTR_POP_F3_HEADER] = (u32)-1,
			[OVS_ACTION_ATTR_POP_F3_AP] = (u32)-1,
			[OVS_ACTION_ATTR_SET_F3_TTL] = (u32)-1,
			[OVS_ACTION_ATTR_DEC_F3_TTL] = (u32)-1,
			[OVS_ACTION_ATTR_F3_JUMP] = (u32)-1,
			[OVS_ACTION_ATTR_F3_FLOOD] = (u32)-1	
		};
		const struct ovs_action_push_vlan *vlan;
		int type = nla_type(a);
		#ifdef OVS_DEBUG
		printk("copy action type = %d %d\n", type, action_lens[type]);
		#endif
		bool skip_copy;
		//printk("%d %d %d\n", OVS_ACTION_ATTR_MAX, nla_len(a), (u32) - 1);

		if (type > OVS_ACTION_ATTR_MAX ||
		    (action_lens[type] != nla_len(a) &&
		     action_lens[type] != (u32)-1)) {
		#ifdef OVS_DEBUG
			printk("ovs_nla_copy_actions in the function 1\n");
		#endif
			return -EINVAL;
		}
		...
		#ifdef OVS_DEBUG
			printk("ovs_nla_copy_actions in the function 2\n");
		#endif
			return -EINVAL;
		...
		#ifdef OVS_DEBUG
				printk("ovs_nla_copy_actions in the function 3\n");
		#endif
				return -EINVAL;
			}
			break;
		...
		#ifdef OVS_DEBUG
				printk("ovs_nla_copy_actions in the function 4\n");
		#endif
				return -EINVAL;
			}
			if (!(vlan->vlan_tci & htons(VLAN_TAG_PRESENT))) {
		#ifdef OVS_DEBUG
				printk("ovs_nla_copy_actions in the function 5\n");
		#endif
				return -EINVAL;
			}
			break;
		...
		case OVS_ACTION_ATTR_SET:
			...
			if (err) {
		#ifdef OVS_DEBUG
				printk("ovs_nla_copy_actions in the function 6\n");
		#endif
				return err;
			}
			break;
		...
		case OVS_ACTION_ATTR_SAMPLE:
			...
			if (err) {
		#ifdef OVS_DEBUG
				printk("ovs_nla_copy_actions in the function 7\n");
		#endif
				return err;
			}
			skip_copy = true;
		...
		case OVS_ACTION_ATTR_PUSH_F3_HEADER: /* Must appear in the edge switch. Nest struct ovs_f3_header_attr. */
		#ifdef OVS_DEBUG
			printk("in the push header!\n");
		#endif
			break;
		case OVS_ACTION_ATTR_POP_F3_HEADER: /* Must appear in the edge switch. NULL*/
		case OVS_ACTION_ATTR_PUSH_F3_AP: /* u32 */
		case OVS_ACTION_ATTR_POP_F3_AP: /* No data. */
		case OVS_ACTION_ATTR_SET_F3_TTL: /* u8 ttl number */
		case OVS_ACTION_ATTR_DEC_F3_TTL: /* NULL */
		case OVS_ACTION_ATTR_F3_JUMP: /*u32 */ 
		...
		if (!skip_copy) {
			...
			if (err) {
		#ifdef OVS_DEBUG
				printk("ovs_nla_copy_actions in the function 8\n");
		#endif
				return err;
			}
		} else {
	...
	if (rem > 0) {
		#ifdef OVS_DEBUG
		printk("ovs_nla_copy_actions in the function 9\n");
		#endif
		return -EINVAL;
	}
...
int copy_f3_action(const struct nlattr *from,
		       struct sw_flow_actions **sfa)
{
	ovs_f3_nla_copy_actions(from, sfa);
	return 0;
}
//**************F3_Modify_Part_End****************************
