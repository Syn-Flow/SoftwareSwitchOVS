...
static struct vport *netdev_create(const struct vport_parms *parms)
{
	...
	port_information = ovs_vport_parse_information(parms);
	if (port_information < 0) {
		err = port_information;
		goto error_put;
	} else {
		vport->port_information = port_information;
	}
	vport->port_type = OVS_VPORT_TYPE_NETDEV;
...
static int net_dev_set_options(struct vport *vport, struct nlattr *attr);
static int net_dev_get_options(const struct vport *vport, struct sk_buff *skb);
const struct vport_ops ovs_netdev_vport_ops = {
	...
	.send		= netdev_send,
	.set_options    = net_dev_set_options,
	.get_options    = net_dev_get_options,
};
...
static int net_dev_set_options(struct vport *vport, struct nlattr *attr) {
	...
	nla_for_each_nested(a, attr, rem) {
		switch (nla_type(a)) {
		case OVS_INFORMATION_ATTR_PORT:
			vport->port_information = nla_get_u32(a);
			#ifdef OVS_DEBUG
			printk("<3>""Net Device Set port no %d, port information %d\n",  vport->port_no, vport->port_information);
			#endif	
			break;
		}
	}
...
static int net_dev_get_options(const struct vport *vport, struct sk_buff *skb)
{
	nla_put_u32(skb, OVS_INFORMATION_ATTR_PORT, vport->port_information);
	return 0;
}
#if LINUX_VERSION_CODE < KERNEL_VERSION(2,6,36) && \
    !defined HAVE_RHEL_OVS_HOOK
