...
ofproto_run(struct ofproto *p)
{
    ...
    default:
        #ifdef DEBUG
        {FILE *f1=fopen("/var/log/ovs_not_reached","ab+");fprintf(f1,"ofproto/ofproto.c");fflush(f1);fclose(f1);OVS_NOT_REACHED();}
	#else
	OVS_NOT_REACHED();
	#endif
    }
...
handle_packet_out(struct ofconn *ofconn, const struct ofp_header *oh)
{
    ...
    //F3
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_handle_packet_out.log","ab+");
    int i, j;
    void *data;
    uint8_t tmp;
    #endif
...
static enum ofperr
handle_ofx_packet_out(struct ofconn *ofconn, const struct ofp_header *oh)
{
    struct ofproto *p = ofconn_get_ofproto(ofconn);
    struct ofputil_packet_out po;
    struct ofpbuf *payload;
    ...
    //F3
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_ofx_packet_out.log","ab+");
    int i,j;
    void *data;
    uint8_t tmp;
    #endif
    ...
    #ifdef DEBUG
    fprintf(fp, "handle_ofx_packet_out 1\n");
    fflush(fp);
    #endif
    error = reject_slave_controller(ofconn);
    if (error) {
    ...
    #ifdef DEBUG
    fprintf(fp, "handle_ofx_packet_out 2\n");
    fflush(fp);
    #endif
    ...
    error = ofputil_decode_packet_out(&po, oh, &ofpacts);
    #ifdef DEBUG
    fprintf(fp, "handle_ofx_packet_out 3, error is %d\n", error);
    fflush(fp);
    #endif
    if (error) {
        goto exit_free_ofpacts;
    ...
    #ifdef DEBUG
    fprintf(fp, "handle_ofx_packet_out 4, error is %d\n", error);
    fflush(fp);
    #endif
    ...
    #ifdef DEBUG
    fprintf(fp, "handle_ofx_packet_out 5, error is %d\n", error);
    fflush(fp);
    #endif
    ...
    error = send_ofx_packet_out(p, payload);
    #ifdef DEBUG
    fprintf(fp, "handle_ofx_packet_out 6, error is %d\n", error);
    fflush(fp);
    #endif
    if(error)
    {
...
/* F3 */
// send port mod reply message to controller
void handle_ofx_port_mod_reply(struct ofconn *ofconn, struct ofproto *ofproto, const struct ofp_header *oh, enum ofperr error, uint32_t port_no, uint32_t config)
{
    struct ofx_port_mod_reply *pmr;
    struct ofpbuf *msg = ofpbuf_new(0);

    ofpbuf_prealloc_tailroom(msg, 32);
    msg->l2 = ofpbuf_put_uninit(msg, 32);
    msg->l3 = ofpbuf_tail(msg);

    pmr = msg->l2;
    pmr->oh.version = oh->version;
    pmr->oh.type = 32;
    pmr->oh.length = htons(msg->size);
    pmr->oh.xid = oh->xid;
    pmr->dpif_id = htonll(ofproto->datapath_id);
    pmr->port_no = htonl(port_no);
    pmr->config = htonl(config);
    pmr->error = error;
    ofconn_send_reply(ofconn, msg);
    return;
}

// update specific port type to kernel 
static enum ofperr
update_port_classifier(struct ofport *port,
                       struct ofconn *ofconn,
                       struct ofproto *ofproto,
                       struct ofp_header *oh,
                       enum ofputil_port_config config)
{
    struct port_classifier pc;
    enum ofperr error = 0;
    switch(config) {
        case OFPUTIL_PC_PORT_STATE: {
            pc.port_information = 1 << 1;
            break;
        }
        case OFPUTIL_PC_PORT_CONFIG: {
            pc.port_information = 1 << 0;
            break;
        }
        case OFPUTIL_PC_PORT_CONFIG|OFPUTIL_PC_PORT_STATE: {
            pc.port_information = 3;
            break;
        }
        default: {
            pc.port_information = 0;
        }
    }
    error = send_ofx_port_msg(port, &pc);
    handle_ofx_port_mod_reply(ofconn, ofproto, oh, error, pc.port_no, pc.port_information);
    return 0;
}
/* F3 */
...
handle_port_mod(struct ofconn *ofconn, const struct ofp_header *oh)
{
    ...
    enum ofperr error;
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_handle_port_mod.log", "ab+");
    #endif
	...
	fclose(fp);
        ...
        // add for F3 port classification 
        if(pm.mask == ((1<<7) + (1<<8))) {
    	    #ifdef DEBUG
	    fprintf(fp, "in the ofx port mod!\n");
	    fflush(fp);
    	    #endif
            error = update_port_classifier(port, ofconn, p, oh, pm.config);
	    #ifdef DEBUG
	    fclose(fp);
	    #endif
            return error;
...
static void
handle_ofx_flow_mod_reply(struct ofconn *ofconn, const struct ofp_header *oh, enum ofperr error, ap_controller action_pointer)
{
    struct ofx_flow_mod_reply *fmr;
    struct ofpbuf *buf = ofpbuf_new(0);
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_ofx_flow_mod_reply.log","ab+");
    #endif

    ofpbuf_prealloc_tailroom(buf,16);
    buf->l2 = ofpbuf_put_uninit(buf, 16);
    buf->l3 = ofpbuf_tail(buf);

    fmr = buf->l2;
    fmr->oh.version = oh->version;
    fmr->oh.type = 31;
    fmr->oh.length = htons(buf->size);
    fmr->oh.xid = oh->xid;
    fmr->action_pointer = action_pointer;
    fmr->error = error;
    #ifdef DEBUG
    fprintf(fp,"error = %hu, ap = %d\n", error, action_pointer);
    fflush(fp);
    #endif
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    long long int now;
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_handle_flow_mod.log", "ab+");
    #endif
    ...
    #ifdef DEBUG
    fprintf(fp, "flow mod end!\n");
    fflush(fp);
    #endif

exit_free_ofpacts:
    #ifdef DEBUG
    fprintf(fp, "free ofpacts begin!\n");
    fflush(fp);
    #endif
    ofpbuf_uninit(&ofpacts);
exit:
    #ifdef DEBUG
    fprintf(fp, "ofx_flow_mod_reply begin!\n");
    fflush(fp);
    fclose(fp);
    #endif
    return error;
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    //F3
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_handle_flow_mod__.log","ab+");
    #endif
    //F3
    ...
    if (ofproto->n_pending < 50) {
        ...
        case OFPFC_ADD:
    	    #ifdef DEBUG
            fprintf(fp, "add flow enter! error = %d\n", error);
            fflush(fp);
    	    #endif
            error = add_flow(ofproto, ofconn, fm, oh);
     	    #ifdef DEBUG
            fprintf(fp, "add flow end! error = %d\n", error);
            fflush(fp);
    	    #endif
            break;
        ...
        case OFPFC_MODIFY:
    	    #ifdef DEBUG
            fprintf(fp, "modify flow enter! error = %d\n", error);
            fflush(fp);
    	    #endif
            error = modify_flows_loose(ofproto, ofconn, fm, oh);
     	    #ifdef DEBUG
            fprintf(fp, "modify flow end! error = %d\n", error);
            fflush(fp);
    	    #endif
            break;
        ...
        case OFPFC_MODIFY_STRICT:
    	    #ifdef DEBUG
            fprintf(fp, "modify strict flow enter! error = %d\n", error);
            fflush(fp);
    	    #endif
            error = modify_flow_strict(ofproto, ofconn, fm, oh);
    	    #ifdef DEBUG
            fprintf(fp, "modify strict flow end! error = %d\n", error);
            fflush(fp);
    	    #endif
            break;
        ...
        case OFPFC_DELETE:
            #ifdef DEBUG
            fprintf(fp, "delete flow enter! error = %d\n", error);
            fflush(fp);
    	    #endif
            error = delete_flows_loose(ofproto, ofconn, fm, oh);
            #ifdef DEBUG
            fprintf(fp, "delete flow end! error = %d\n", error);
            fflush(fp);
    	    #endif
            break;
        ...
        case OFPFC_DELETE_STRICT:
            #ifdef DEBUG
            fprintf(fp, "delete strict flow enter! error = %d\n", error);
            fflush(fp);
    	    #endif
            error = delete_flow_strict(ofproto, ofconn, fm, oh);
            #ifdef DEBUG
            fprintf(fp, "delete strict flow end! error = %d\n", error);
            fflush(fp);
    	    #endif
            break;
    ...
    #ifdef DEBUG
    fprintf(fp, "end of the function!\n");
    fflush(fp);
    fclose(fp);
    #endif
    return error;
...
static enum ofperr
handle_ofx_flow_mod(struct ofconn *ofconn, const struct ofp_header *oh)
{
    int error = 0;
    struct ofproto *ofproto = ofconn_get_ofproto(ofconn);
    const struct ofx_header *oxh; 
    struct ofpbuf msg, ofpacts;
    struct ofx_flow_mod *ofm;
    struct ofputil_flow_mod fm;
    uint64_t ofpacts_stub[1024 / 8];
    ap_controller action_pointer = 0;
    enum ofpraw raw;
    //F3
    #ifdef DEBUG
    uint8_t tmp;
    int i,j;
    void *data;
    FILE *fp = fopen("/var/log/ovs_ofx_flow_mod.log","a+");
    #endif
    //F3

    memset(&(fm.match), 0, sizeof(struct match));
    #ifdef DEBUG
    fprintf(fp, "match size %d\n", sizeof(struct match));
    fflush(fp);
    #endif
    ofpbuf_use_stub(&ofpacts, ofpacts_stub, sizeof ofpacts_stub);
    ofpbuf_use_const(&msg, oh, ntohs(oh->length));
    #ifdef DEBUG
    fprintf(fp, "enter the function!\n");
    fflush(fp);
    #endif
    ...
    ofm = ofpbuf_pull(&msg, sizeof *ofm);
    ...
    #endif
    //F3
    error = ofpacts_pull_openflow_instructions(&msg, msg.size, oh->version, &ofpacts);
    #ifdef DEBUG
    fprintf(fp,"message length = %d\n", msg.size);
    fflush(fp);
    #endif
    if (error) {
    	    #ifdef DEBUG
	    fprintf(fp, "error in instructions part!\n");
	    fflush(fp);
    	    #endif
	    goto exit;
    }
    #ifdef DEBUG
    fprintf(fp,"enter the send module!\n");
    fflush(fp);
    #endif
    error = send_ofx_flow_mod_msg(ofproto, &ofpacts, &fm.match.flow, &action_pointer);

    /* TODO: analyse the rule and send it to kernel */

exit:
    #ifdef DEBUG
    fprintf(fp, "enter the reply module!\n");
    fflush(fp);
    fclose(fp);
    #endif
    handle_ofx_flow_mod_reply(ofconn, oh, error, action_pointer);
    return 0;
}
    ...
    OVS_EXCLUDED(ofproto_mutex)
{
    ...
    case OFPTYPE_OFX_PACKET_OUT:
        //return 0;
        return handle_ofx_packet_out(ofconn, oh);
    ...
    case OFPTYPE_OFX_FLOW_MOD:
        return handle_ofx_flow_mod(ofconn, oh);
...
ofoperation_complete(struct ofoperation *op, enum ofperr error)
{
    ...
    //F3
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_ofoperation_complete.log", "ab+");
    #endif
    //F3
    
    #ifdef DEBUG
    fprintf(fp, "function enter!\n");
    fflush(fp);
    #endif
    ...
    #ifdef DEBUG
    fprintf(fp, "function finish error = %d\n", error);
    fflush(fp);
    fclose(fp);
    #endif
}
