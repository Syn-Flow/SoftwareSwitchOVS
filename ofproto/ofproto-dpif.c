                  ...
                  const struct ofpbuf *ofpacts, struct rule_dpif **rulep)
{
    ...
    if (rule_dpif_lookup_in_table(ofproto, &fm.match.flow, NULL, TBL_INTERNAL,
        ...
        {FILE *f1=fopen("/var/log/ovs_not_reached","ab+");fprintf(f1,"ofproto/ofproto-dpif.c");fflush(f1);fclose(f1);OVS_NOT_REACHED();}
    }
...
port_reconfigured(struct ofport *port_, enum ofputil_port_config old_config)
{
    ...
    if (changed & (OFPUTIL_PC_NO_RECV | OFPUTIL_PC_NO_RECV_STP |
                   ...
                   OFPUTIL_PC_NO_PACKET_IN | OFPUTIL_PC_PORT_STATE | OFPUTIL_PC_PORT_CONFIG)) {
        ofproto->backer->need_revalidate = REV_RECONFIGURE;
...
int 
send_ofx_port_msg(const struct ofport *port_, struct port_classifier *pc)
{
    struct ofport_dpif *port = ofport_dpif_cast(port_);
    struct ofproto_dpif *ofproto = ofproto_dpif_cast(port->up.ofproto);
    int error;
    pc->port_no = port->odp_port;
    pc->dp_ifindex = dpif_linux_get_dpifindex(ofproto->backer->dpif); 
    error = dpif_linux_get_upcallpid(ofproto->backer->dpif, pc->port_no, &(pc->upcall_pid)); 
    if(error) {
        return error;
    }
    error = dpif_send_ofx_port_msg(ofproto->backer->dpif, pc, port_->netdev);
    return error;
}

int 
send_ofx_flow_mod_msg(const struct ofproto *ofproto_, struct ofpbuf *ofpacts, struct flow *flow, ap_controller *action_pointer)
{
    int error;
    struct ofproto_dpif *ofproto = ofproto_dpif_cast(ofproto_);
    error = ofx_flow_mod_transfer_and_send(ofproto, ofpacts, flow, action_pointer, ofproto->backer->dpif);
    return error;
}

int 
send_ofx_packet_out(const struct ofproto *ofproto_, struct ofpbuf *payload)
{
    int error;
    int dp_ifindex;
    struct ofproto_dpif *ofproto = ofproto_dpif_cast(ofproto_);
    dp_ifindex = dpif_linux_get_dpifindex(ofproto->backer->dpif); 
    error = dpif_send_ofx_packet_out(dp_ifindex, payload);
    return error;
}
