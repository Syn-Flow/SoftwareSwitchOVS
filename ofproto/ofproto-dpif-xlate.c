...
static void do_xlate_actions(const struct ofpact *, size_t ofpacts_len,
                             struct xlate_ctx *);
void do_xlate_ofx_actions(const struct ofpact *ofpacts, size_t ofpacts_len, struct xlate_ctx *ctx, ap_controller *action_pointer);
static void xlate_actions__(struct xlate_in *, struct xlate_out *)
    OVS_REQ_RDLOCK(xlate_rwlock);
void xlate_ofx_actions__(struct xlate_in *xin, struct xlate_out *xout, struct ofpbuf *ofpacts, ap_controller *action_pointer)
    OVS_REQ_RDLOCK(xlate_rwlock);
              ...
              odp_port_t *odp_in_port)
{
    ...
    int error = ENODEV;
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_xlate_receive.log", "ab+");
    #endif 
    ...
    if (fitness == ODP_FIT_ERROR) {
        error = EINVAL;
    	#ifdef DEBUG
        fprintf(fp, "odp_flow_key_to_flow error!\n");
        fflush(fp);
	#endif
        goto exit;
    }
    ...
    #ifdef DEBUG
    fprintf(fp, "xport is %d\n", (xport==NULL));
    fflush(fp);
    #endif

    flow->in_port.ofp_port = xport ? xport->ofp_port : OFPP_NONE;
    #ifdef DEBUG
    fprintf(fp, "xport ofp_port = %d", flow->in_port.ofp_port);
    fflush(fp);
    #endif
    if (!xport) {
        goto exit;
    ...
    if (ofproto) {
        *ofproto = xport->xbridge->ofproto;
    	#ifdef DEBUG
	fprintf(fp, "has found the ofproto_dpif!\n");
	fflush(fp);
    	#endif
    }
...
input_vid_to_vlan(const struct xbundle *in_xbundle, uint16_t vid)
{
    ...
    default:
    	#ifdef DEBUG
        {FILE *f1=fopen("/var/log/ovs_not_reached","ab+");fprintf(f1,"ofproto/ofproto-dpif-xlate.c");fflush(f1);fclose(f1);OVS_NOT_REACHED();}
	#else
	OVS_NOT_REACHED();
    	#endif
    }
                        ...
                        bool check_stp)
{
    ...
    uint8_t dscp;
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_compose_output_action__.log","ab+");
    fprintf(fp, "in function 1, ofp_port = %d\n", ofp_port);
    fflush(fp);
    #endif 
    ...
    if (mbridge_has_mirrors(ctx->xbridge->mbridge) && xport->xbundle) {
    	#ifdef DEBUG
        fprintf(fp, "in function 2, ofp_port = %d\n", ofp_port);
        fflush(fp);
    	#endif
    ...
    if (xport->peer) {
    	...
    	#ifdef DEBUG
        fprintf(fp, "in function 3, ofp_port = %d\n", ofp_port);
        fflush(fp);
    	#endif
    ...
    if (xport->is_tunnel) {
        ...
        commit_odp_tunnel_action(flow, &ctx->base_flow,
                                 &ctx->xout->odp_actions);
    	#ifdef DEBUG	
	fprintf(fp, "in function 4, ofp_port = %d\n", ofp_port);
	fflush(fp);
    	#endif
    	...
    	#ifdef DEBUG
        fprintf(fp, "in function 4.5, odp_port = %d\n", odp_port);
        fflush(fp);
    	#endif
    	...
    	#ifdef DEBUG
        fprintf(fp, "in function 4.6, ofp_port = %d, odp_port = %d\n", ofp_port, odp_port);
        fflush(fp);
    	#endif
            out_port = odp_port;
        } else {
    	#ifdef DEBUG
        fprintf(fp, "in function 4.7, ofp_port = %d, odp_port = %d\n", ofp_port, odp_port);
        fflush(fp);
    	#endif
    ...
    if (out_port != ODPP_NONE) {
    	...
    	#ifdef DEBUG
        fprintf(fp, "in function 5, ofp_port = %d, out_port = %d\n", ofp_port, out_port);
        fflush(fp);
    	#endif
...
ap_controller 
xlate_f3_actions(struct xlate_ctx *ctx,
                 const void *data,
                 ...
                 bool flag)
{
    ...
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_xlate_f3_actions.log","ab+");
    #endif 
    switch(type) {
        case OFPACT_OFX_PUSH_HEADER: {
            struct ofpact_ofx_push_header *oph;
    	    #ifdef DEBUG
	    void *data__;
	    uint8_t tmp;
            int i,j;
    	    #endif
	    size_t f3_offset, size;
            oph = (struct ofpact_ofx_push_header *)data;
    	    #ifdef DEBUG
	    data__ = oph;
	    for(i = 0; i < oph->ofpact.len;i++)
	    {
		    tmp = *((uint8_t *)data__);
		    data__ = data__ + 1;
		    for(j = 0; j < 8; j++)
		    {
			    fprintf(fp, "%d",((tmp&(1<<(7-j))) > 0));
			    fflush(fp);
		    }
		    fprintf(fp," ");
		    fflush(fp);
	    ...
	    fflush(fp);
    	    #endif
            size = oph->action_pointer_number * sizeof(ap_controller);
    	    #ifdef DEBUG
	    fprintf(fp, "ttl %d, #ap %d, len %d\n", oph->ttl, oph->action_pointer_number, oph->ofpact.len);
	    fflush(fp);
    	    #endif
            f3_offset = nl_msg_start_nested(odp_actions, OVS_ACTION_ATTR_PUSH_F3_HEADER);
            nl_msg_put_u8(odp_actions, OVS_HEADER_ATTR_TTL, oph->ttl);
            nl_msg_put_u8(odp_actions, OVS_HEADER_ATTR_NUMBER, oph->action_pointer_number);
            nl_msg_put_unspec(odp_actions, OVS_HEADER_ATTR_AP, oph->action_pointer, size);
            nl_msg_end_nested(odp_actions, f3_offset);
            break;
        }
        case OFPACT_OFX_PUSH_AP: {
            struct ofpact_ofx_push_ap *opa;
            opa = (struct ofpact_ofx_push_ap *)data;
            nl_msg_put_u16(odp_actions, OVS_ACTION_ATTR_PUSH_F3_AP, opa->action_pointer);
            break;
        }
        case OFPACT_OFX_SET_TTL: {
            struct ofpact_ofx_set_ttl *ost;
            ost = (struct ofpact_ofx_set_ttl *)data;
            nl_msg_put_u8(odp_actions, OVS_ACTION_ATTR_SET_F3_TTL, ost->ttl);
            break;
        }
        case OFPACT_OFX_JUMP: {
            struct ofpact_ofx_jump *oj;
            oj = (struct ofpact_ofx_jump *)data;
	    if(flag) {
	        return oj->action_pointer;
	    }
	    else {
		nl_msg_put_u16(odp_actions, OVS_ACTION_ATTR_F3_JUMP, oj->action_pointer);
		return oj->action_pointer;
	    }
        }
        case OFPACT_OFX_POP_HEADER: 
            nl_msg_put_flag(odp_actions, OVS_ACTION_ATTR_POP_F3_HEADER);
	    break;
        case OFPACT_OFX_POP_AP: 
            nl_msg_put_flag(odp_actions, OVS_ACTION_ATTR_POP_F3_AP);
            break;
        case OFPACT_OFX_DEC_TTL: {
            nl_msg_put_flag(odp_actions, OVS_ACTION_ATTR_DEC_F3_TTL);
            break;
        } 
        default :
        {
                 ...
                 struct xlate_ctx *ctx)
{
    ...
    //F3
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_do_xlate_actions.log", "ab+");
    #endif 
    //F3
    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
        ...
        case OFPACT_DEC_TTL:
            ...
            break;
        /* F3. */
        // Append F3 actions to odp_actions 
        case OFPACT_OFX_PUSH_HEADER:
    	    #ifdef DEBUG
            fprintf(fp, "begin translate the f3 aciton! 1\n");
            fflush(fp);
    	    #endif
            xlate_f3_actions(ctx, ofpact_get_OFX_PUSH_HEADER(a), a->type, false);
    	    #ifdef DEBUG
            fprintf(fp, "end translate the f3 aciton! 1\n");
            fflush(fp);
    	    #endif
            break;
        ...
        case OFPACT_OFX_POP_HEADER:
    	    #ifdef DEBUG
            fprintf(fp, "begin translate the f3 aciton! 2\n");
            fflush(fp);
    	    #endif
            xlate_f3_actions(ctx, ofpact_get_OFX_POP_HEADER(a), a->type, false);
    	    #ifdef DEBUG
            fprintf(fp, "end translate the f3 aciton! 2\n");
            fflush(fp);
    	    #endif

        case OFPACT_OFX_PUSH_AP:
    	    #ifdef DEBUG
            fprintf(fp, "begin translate the f3 aciton! 3\n");
            fflush(fp);
    	    #endif
            xlate_f3_actions(ctx, ofpact_get_OFX_PUSH_AP(a), a->type, false);
    	    #ifdef DEBUG
            fprintf(fp, "end translate the f3 aciton! 3\n");
            fflush(fp);
    	    #endif
            break;
        ...
        case OFPACT_OFX_POP_AP:
    	    #ifdef DEBUG
            fprintf(fp, "begin translate the f3 aciton! 4\n");
            fflush(fp);
    	    #endif
            xlate_f3_actions(ctx, ofpact_get_OFX_POP_AP(a), a->type, false);
    	    #ifdef DEBUG
            fprintf(fp, "end translate the f3 aciton! 4\n");
            fflush(fp);
    	    #endif
            break;
        ...
        case OFPACT_OFX_SET_TTL:
    	    #ifdef DEBUG
            fprintf(fp, "begin translate the f3 aciton! 5\n");
            fflush(fp);
    	    #endif
            xlate_f3_actions(ctx, ofpact_get_OFX_SET_TTL(a), a->type, false);
    	    #ifdef DEBUG
            fprintf(fp, "end translate the f3 aciton! 5\n");
            fflush(fp);
    	    #endif
            break;
        ...
        case OFPACT_OFX_DEC_TTL:
    	    #ifdef DEBUG
            fprintf(fp, "begin translate the f3 aciton! 6\n");
            fflush(fp);
    	    #endif
            xlate_f3_actions(ctx, ofpact_get_OFX_DEC_TTL(a), a->type, false);
    	    #ifdef DEBUG
            fprintf(fp, "end translate the f3 aciton! 6\n");
            fflush(fp);
    	    #endif
            break;
        ...
        case OFPACT_OFX_JUMP:
    	    #ifdef DEBUG
            fprintf(fp, "begin translate the f3 aciton! 7\n");
            fflush(fp);
    	    #endif
            xlate_f3_actions(ctx, ofpact_get_OFX_JUMP(a), a->type, false);
    	    #ifdef DEBUG
            fprintf(fp, "end translate the f3 aciton! 7\n");
            fflush(fp);
    	    #endif
            break;
        /* F3. */
...
/* F3 */
static void
xlate_ofx_output_action(struct xlate_ctx *ctx,
                    ofp_port_t port, uint16_t max_len, bool may_packet_in, ap_controller action_pointer)
{
    struct xport *xport = get_ofp_port(ctx->xbridge, port);
    size_t offset;
    odp_port_t odp_port;
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_xlate_ofx_output_action.log","ab+"); 
    #endif 
    if (xport == NULL) {
         odp_port = 0;
    } else {
    ...
    #ifdef DEBUG
    fprintf(fp, "port = %d, INPORT = %d, TABLE = %d, NORMAL = %d, OFPP_FLOOD = %d, OFPP_ALL = %d, OFPP_CONTROLLER = %d, NONE = %d, LOCAL = %d\n", port, OFPP_IN_PORT, OFPP_TABLE, OFPP_NORMAL, OFPP_FLOOD, OFPP_ALL, OFPP_CONTROLLER, OFPP_NONE, OFPP_LOCAL);
    fflush(fp);
    #endif
    switch (port) {
    case OFPP_IN_PORT:
    	#ifdef DEBUG
	fprintf(fp, "in function 1\n");
	fflush(fp);
    	#endif
        compose_output_action(ctx, ctx->xin->flow.in_port.ofp_port);
        break;
    case OFPP_TABLE:
    	#ifdef DEBUG
	fprintf(fp, "in function 2\n");
	fflush(fp);
    	#endif
        break;
    case OFPP_NORMAL:
    	#ifdef DEBUG
	fprintf(fp, "in function 3\n");
	fflush(fp);
    	#endif
        break;
    case OFPP_FLOOD:
    	#ifdef DEBUG
	fprintf(fp, "in function 4\n");
	fflush(fp);
    	#endif
        flood_packets(ctx,  false);
        break;
	...
	commit_odp_actions(&ctx->xin->flow, &ctx->base_flow,
			   ...
			   &ctx->xout->wc,
			   &ctx->mpls_depth_delta);
        offset = nl_msg_start_nested(&ctx->xout->odp_actions, OVS_ACTION_ATTR_USERSPACE);
        nl_msg_put_u32(&ctx->xout->odp_actions, OVS_USERSPACE_ATTR_PID, 0);
	nl_msg_end_nested(&ctx->xout->odp_actions, offset);
        break;
    case OFPP_NONE:
    	#ifdef DEBUG
	fprintf(fp, "in function 7\n");
	fflush(fp);
    	#endif
        break;
    case OFPP_LOCAL:
    	#ifdef DEBUG
	fprintf(fp, "in function 8\n");
	fflush(fp);
    	#endif
    default:
    	#ifdef DEBUG
	fprintf(fp, "in function 9\n");
	fflush(fp);
	fprintf(fp, "default port = %d, odp_port = %d, action_pointer = %d\n", port, odp_port, action_pointer);
	fflush(fp);
    	#endif
	compose_output_action(ctx, port);
    	#ifdef DEBUG
	fprintf(fp, "default port = %d\n", port);
	fflush(fp);
    	#endif
        break;
    }
...
void do_xlate_ofx_actions(const struct ofpact *ofpacts, size_t ofpacts_len, struct xlate_ctx *ctx, ap_controller *action_pointer)
{
    ...
    bool ap_flag = true;
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_do_xlate_ofx_actions.log", "ab+");
    #endif 
    ...
    OFPACT_FOR_EACH (a, ofpacts, ofpacts_len) {
    	...
    	#ifdef DEBUG
	fprintf(fp, "in the processing module, the type is %d\n", a->type);
	fflush(fp);
    	#endif
        ...
        case OFPACT_OUTPUT:
    		#ifdef DEBUG
		fprintf(fp, "in the output processing module 1, port_no is %d\n", ofpact_get_OUTPUT(a)->port);
		fflush(fp);
    		#endif
		xlate_ofx_output_action(ctx, ofpact_get_OUTPUT(a)->port,
                                ofpact_get_OUTPUT(a)->max_len, true, *action_pointer);
    		#ifdef DEBUG
		fprintf(fp, "in the output processing module 2\n");
		fflush(fp);
    		#endif
            break;
        ...
        case OFPACT_SET_FIELD:
            #ifdef DEBUG
	    fprintf(fp, "in the set field function!\n");
	    fflush(fp);
    	    #endif
            set_field = ofpact_get_SET_FIELD(a);
            mf = set_field->field;
        ...
        case OFPACT_OFX_POP_HEADER:
            #ifdef DEBUG
            fprintf(fp, "in the pop shim processing\n");
            fflush(fp);
    	    #endif
            xlate_f3_actions(ctx, ofpact_get_OFX_POP_HEADER(a), a->type, false);
            break;
        ...
        case OFPACT_OFX_JUMP:
            #ifdef DEBUG
	    fprintf(fp, "in the jump processing, ap_flag = %d, action_pointer=%d\n",ap_flag, *action_pointer);
	    fflush(fp);
    	    #endif
	    if(ap_flag && !(*action_pointer)) {
                *action_pointer = xlate_f3_actions(ctx, ofpact_get_OFX_JUMP(a), a->type, true);
		ap_flag = false;
            	#ifdef DEBUG
                fprintf(fp, "action pointer = %d\n", *action_pointer);
		fflush(fp);
    	        #endif
                break;
	    } else {
...
void 
xlate_ofx_actions_set(struct xlate_ctx *ctx, ap_controller *action_pointer)
{
    uint64_t action_list_stub[1024 / 64];
    struct ofpbuf action_list;
    ...
    ofpbuf_use_stub(&action_list, action_list_stub, sizeof action_list_stub);
    ofpacts_execute_action_set(&action_list, &ctx->action_set);
    do_xlate_ofx_actions(action_list.data, action_list.size, ctx, action_pointer);
    ofpbuf_uninit(&action_list);
}

void
xlate_ofx_actions(struct xlate_in *xin, struct xlate_out *xout, struct ofpbuf *ofpacts, ap_controller *action_pointer)
{
    ovs_rwlock_rdlock(&xlate_rwlock);
    xlate_ofx_actions__(xin, xout, ofpacts, action_pointer);
    ovs_rwlock_unlock(&xlate_rwlock);
}

void 
xlate_ofx_actions__(struct xlate_in *xin, struct xlate_out *xout, struct ofpbuf *ofpacts, ap_controller *action_pointer)
    OVS_REQ_RDLOCK(xlate_rwlock)
{
    struct flow_wildcards *wc = &xout->wc;
    struct flow *flow = &xin->flow;
    struct xlate_ctx ctx;
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_xlate_ofx_actions.log", "ab+");
    #endif 
    
    #ifdef DEBUG
    fprintf(fp,"in the function 1\n");
    fflush(fp);
    #endif
    ctx.xin = xin;
    ctx.xout = xout;
    ...
    ctx.xbridge = xbridge_lookup(xin->ofproto);
    #ifdef DEBUG
    fprintf(fp,"in the function 2, ctx.xbridge is %d\n", (ctx.xbridge==NULL));
    fflush(fp);
    #endif
    ...
    #ifdef DEBUG
    fprintf(fp,"in the function 3, action_pointer = %d\n", *action_pointer);
    fflush(fp);
    #endif
    do_xlate_ofx_actions(ofpacts->data, ofpacts->size, &ctx, action_pointer);
    #ifdef DEBUG
    fprintf(fp,"in the function 4, action_pointer = %d\n", *action_pointer);
    fflush(fp);
    #endif
    xlate_ofx_actions_set(&ctx, action_pointer);
    #ifdef DEBUG
    fprintf(fp,"in the function 5, action_pointer = %d\n", *action_pointer);
    fflush(fp);
    fclose(fp);
    #endif
