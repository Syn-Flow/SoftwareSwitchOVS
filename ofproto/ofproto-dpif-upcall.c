...
#include "include/openflow/ofx-ext.h"
...
udpif_upcall_handler(void *arg)
{
    ...
    //FILE *fp = fopen("/var/log/ovs_udpif_upcall_handler.log", "ab+");
...
classify_upcall(const struct upcall *upcall)
{
    ...
    //F3
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_classify_upcall.log", "ab+");
    ...
    case DPIF_UC_OFX_MISS:
    	return OFX_MISS_UPCALL;
    ...
    case DPIF_UC_OFX_ACTION:
	return OFX_ACTION_UPCALL; 
...
handle_upcalls(struct handler *handler, struct list *upcalls)
{
    ...
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_handle_ofx_miss_upcall.log","ab+");
    #endif
    ...
    LIST_FOR_EACH_SAFE (upcall, next, list_node, upcalls) {
        ...
        if (type == MISS_UPCALL) {
    	    ...
    	    #ifdef DEBUG
            fprintf(fp,"in the original upcall processing module\n");
            fflush(fp);
            #endif
        ...
        switch (type) {
        case OFX_MISS_UPCALL: { 
	    struct ofproto_packet_in *pin;

	    pin = xmalloc(sizeof *pin);
            #ifdef DEBUG
            fprintf(fp,"in the ofx miss upcall processing module 1\n");
    	    fflush(fp);
	    fprintf(fp, "packet_len = %d\n", packet->size);
	    fflush(fp);
            #endif
	    pin->up.packet = xmemdup(packet->data, packet->size);
	    pin->up.packet_len = packet->size;
	    pin->up.reason = OFPR_OFX_FLOW_MISS;
	    pin->up.table_id = 0;
	    ...
	    ofproto_dpif_send_packet_in(ofproto, pin);
            #ifdef DEBUG
            fprintf(fp,"in the ofx miss upcall processing module 2\n");
	    fflush(fp);
            #endif
	    break;
	    }
        case OFX_ACTION_UPCALL: { 
	    struct ofproto_packet_in *pin;
        ...
        case MISS_UPCALL:
            #ifdef DEBUG
            {FILE *f1=fopen("/var/log/ovs_not_reached","ab+");fprintf(f1,"ofproto/ofproto-dpif-upcall.c");fflush(f1);fclose(f1);OVS_NOT_REACHED();}
	    #else
	    OVS_NOT_REACHED();
            #endif
        }
    ...
    #ifdef DEBUG
    fprintf(fp, "number of operation is %d\n", n_ops);
    fflush(fp);
    fclose(fp);
    #endif
...
int 
ofx_flow_mod_transfer_and_send(struct ofproto_dpif *ofproto, struct ofpbuf *ofpacts, struct flow *flow, ap_controller *action_pointer, struct dpif *dpif)
{
    struct dpif_op op;	
    struct xlate_in xin;
    struct xlate_out xout;
    struct nl_key_ap ka;
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_ofx_send_flow_mod_to_kernel.log","a+");
    #endif
    xlate_in_init(&xin, ofproto, flow, NULL, 0, NULL);
    xin.may_learn = true;
    #ifdef DEBUG
    ...
    //TODO: translate the ofpact from ofpacts to xout->odp_actions
    xlate_ofx_actions(&xin, &xout, ofpacts, action_pointer);
    #ifdef DEBUG
    fprintf(fp, "in the function 2.5, action_pointer = %d\n", *action_pointer);
    fflush(fp);
    #endif
    ka.action_pointer = *action_pointer;
    #ifdef DEBUG
    ...
    op.type = DPIF_OP_FLOW_PUT;
    op.u.flow_put.flags = DPIF_FP_CREATE | DPIF_FP_MODIFY;
    op.u.flow_put.key = &ka;
    op.u.flow_put.key_len = sizeof(struct nl_key_ap);
    op.u.flow_put.mask = NULL;
    op.u.flow_put.mask_len = 0;
    op.u.flow_put.stats = NULL;
    op.u.flow_put.actions = xout.odp_actions.data;
    op.u.flow_put.actions_len = xout.odp_actions.size;

    #ifdef DEBUG
    fprintf(fp,"key_len = %d, actions_len = %d\n", op.u.flow_put.key_len, op.u.flow_put.actions_len);
    fflush(fp);
    #endif
    dpif_ofx_flow_put(dpif, &op);
    #ifdef DEBUG
    fprintf(fp, "in the function 4\n");
    fflush(fp);
    #endif
    #else
    install_flow_on_hw(action_pointer, xout.odp_actions.data, xout.odp_actions.data);
    #endif

    xlate_out_uninit(&xout);
    #ifdef DEBUG
    fprintf(fp, "in the function 5\n");
    fflush(fp);
    fclose(fp);
    #endif
    return op.error;
