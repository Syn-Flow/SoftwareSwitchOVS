...
/* F3 */
int send_ofx_port_msg(const struct ofport *port_, struct port_classifier *pc);
int send_ofx_flow_mod_msg(const struct ofproto *ofproto_, struct ofpbuf *ofpacts, struct flow *flow, ap_controller *action_pointer);
int send_ofx_packet_out(const struct ofproto *ofproto_, struct ofpbuf *payload);
