    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    if (ofconn->enable_async_msgs) {
         ...
         * reasons itself. */
        master[OAM_PACKET_IN] = (1u << OFPR_NO_MATCH) | (1u << OFPR_ACTION) | (1u << OFPR_OFX_FLOW_MISS);
                          ...
                          unsigned int reason)
{
    ...
    #ifdef DEBUG
    FILE *fp =  fopen("/var/log/ovs_receives_asyns_msg.log", "ab+");
    #endif
    ...
    #ifdef DEBUG
    fprintf(fp, "protocol is %d, connection state is %d\n", ofconn_get_protocol(ofconn), rconn_is_connected(ofconn->rconn));
    #endif
    ...
    #ifdef DEBUG
    fprintf(fp, "switch role is %d, async_config[packet_in] = %d\n", (ofconn->role == OFPCR12_ROLE_SLAVE), async_config[type]);
    fflush(fp);
    #endif
                       ...
                       const struct ofproto_packet_in *pin)
{
    struct ofconn *ofconn;
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_connmgr_send_packet_in.log","ab+");
    #endif

    LIST_FOR_EACH (ofconn, node, &mgr->all_conns) {
        enum ofp_packet_in_reason reason = wire_reason(ofconn, pin);
        #ifdef DEBUG
        fprintf(fp, "connmgr_send_packet_in begin! controller id is %d, pin controller id is %d, reason is %d\n", ofconn->controller_id, pin->controller_id, reason);
        fflush(fp);
        #endif
        ...
        #ifdef DEBUG
        fprintf(fp, "connmgr_send_packet_in end!\n");
        fflush(fp);
        #endif
    }
    #ifdef DEBUG
                   ...
                   enum ofp_packet_in_reason wire_reason)
{
    ...
    struct list txq;
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_schedule_packet_in.log", "ab+");
    #endif
    ...
    pinsched_send(ofconn->schedulers[pin.up.reason == OFPR_NO_MATCH ? 0 : 1],
		  ...
		  &txq);
    #ifdef DEBUG
    fprintf(fp, "do_send_packet_in begin!\n");
    fflush(fp);
    #endif
    do_send_packet_ins(ofconn, &txq);
    #ifdef DEBUG
    fprintf(fp, "do_send_packet_in end!\n");
    fflush(fp);
    fclose(fp);
    #endif
}
    ...
    OVS_REQUIRES(ofproto_mutex)
{
    ...
    case NXFME_ABBREV:
        #ifdef DEBUG
        {FILE *f1=fopen("/var/log/ovs_not_reached","ab+");fprintf(f1,"ofproto/connmgr.c");fflush(f1);fclose(f1);OVS_NOT_REACHED();}
	#else
	OVS_NOT_REACHED();
        #endif
    }
