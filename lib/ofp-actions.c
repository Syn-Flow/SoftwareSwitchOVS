...
union ofp_action {
	...
	struct ofp11_action_ofx_push_header ofx_push_header;
	struct ofp11_action_ofx_push_ap ofx_push_ap;
	struct ofp11_action_ofx_set_ttl ofx_set_ttl;
	struct ofp11_action_ofx_jump ofx_jump;
};
...
/* F3 */
// parse ofx push action message
static void
ofx_push_header_from_openflow(const struct ofp11_action_ofx_push_header *oph, struct ofpbuf *out)
{
	struct ofpact_ofx_push_header *ooph;
	unsigned int length;
        int i, j;
	#ifdef DEBUG
	void *data;
	uint8_t tmp;
	FILE *fp = fopen("/var/log/ovs_ofx_push_header_from_openflow.log","ab+");   

	//F3
	data = oph;
	for(i = 0; i < ntohs(oph->len);i++)
	{
		tmp = *((uint8_t *)data);
		data = data + 1;
		for(j = 0; j < 8; j++)
	...
	ooph = ofpact_put_OFX_PUSH_HEADER(out);
	ooph->action_pointer_number = oph->action_pointer_number;
	ooph->ttl = oph->ttl;
	//F3
	#ifdef DEBUG
	fprintf(fp, "ooph length %d\n", ooph->ofpact.len);
	data = ooph;
	for(i = 0; i < ooph->ofpact.len;i++)
	{
		tmp = *((uint8_t *)data);
		data = data + 1;
		for(j = 0; j < 8; j++)
	...
	for (i = 0; i < ooph->action_pointer_number; i++) {
		ap_kernel ap = ntohs(((ap_controller *)(oph + 1))[i]);
		ofpbuf_put(out, &ap, sizeof ap);
		ooph = out->l2;
	}
	ofpact_update_len(out, &ooph->ofpact);
	//F3
	#ifdef DEBUG
	fprintf(fp, "ooph length %d\n", ooph->ofpact.len);
	data = ooph;
...
#define NXAST_ACTION(ENUM, STRUCT, EXTENSIBLE, NAME)    \
        case CONSTANT_HTONS(ENUM):                      \
            ...
            {FILE *f1=fopen("/var/log/ovs_not_reached","ab+");fprintf(f1,"lib/ofp-actions.c 0");fflush(f1);fclose(f1);OVS_NOT_REACHED();}
...
#include "ofp-util.def"
	#ifdef DEBUG
        {FILE *f1=fopen("/var/log/ovs_not_reached","ab+");fprintf(f1,"lib/ofp-actions.c 1");fflush(f1);fclose(f1);OVS_NOT_REACHED();}
	#else
	OVS_NOT_REACHED();
	#endif
...
#include "ofp-util.def"
	#ifdef DEBUG
        {FILE *f1=fopen("/var/log/ovs_not_reached","ab+");fprintf(f1,"lib/ofp-actions.c, 2");fflush(f1);fclose(f1);OVS_NOT_REACHED();}
	#else
	OVS_NOT_REACHED();
	#endif
                      ...
                      enum ofp_version version, struct ofpbuf *out)
{
    ...
    size_t left;
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_ofpacts_from_openflow.log","ab+");
    #endif
    ...
    #ifdef DEBUG
    fprintf(fp,"version is %d\n", version);
    fflush(fp);
    #endif
    ACTION_FOR_EACH (a, left, in, n_in) {
    	#ifdef DEBUG
        fprintf(fp,"enter the action_for_each function 1\n");
	fflush(fp);
	#endif
        enum ofperr error = ofpact_from_openflow(a, version, out);
    	#ifdef DEBUG
        fprintf(fp,"enter the action_for_each function 2\n");
	fflush(fp);
	#endif
        if (error) {
    		#ifdef DEBUG
		fprintf(fp,"enter the action_for_each function 3\n");
		fflush(fp);
                fclose(fp);
		#endif
...
#define OFPAT11_ACTION(ENUM, STRUCT, EXTENSIBLE, NAME)  \
        case CONSTANT_HTONS(ENUM):                      \
            ...
            {FILE *f1=fopen("/var/log/ovs_not_reached","ab+");fprintf(f1,"lib/ofp-actions.c 3");fflush(f1);fclose(f1);OVS_NOT_REACHED();}
#include "ofp-util.def"
                      ...
                      struct ofpbuf *openflow)
{
    ...
    if (oh->version >= OFP12_VERSION) {
    	...
    	#ifdef DEBUG
        {FILE *f1=fopen("/var/log/ovs_not_reached","ab+");fprintf(f1,"lib/ofp-actions.c 4");fflush(f1);fclose(f1);OVS_NOT_REACHED();}
	#else
	OVS_NOT_REACHED();
	#endif
    }
                       ...
                       struct ofpbuf *out)
{
    ...
    struct ofpact_vlan_pcp *vlan_pcp;
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_ofpact_from_openflow11.log","ab+");
    #endif

    error = decode_openflow11_action(a, &code);
    #ifdef DEBUG
    fprintf(fp, "error is %d code is %d\n",error, code);
    fflush(fp);
    fclose(fp);
    #endif
    if (error) {
...
#include "ofp-util.def"
    	#ifdef DEBUG
        {FILE *f1=fopen("/var/log/ovs_not_reached","ab+");fprintf(f1,"lib/ofp-actions.c 5");fflush(f1);fclose(f1);OVS_NOT_REACHED();}
	#else
	OVS_NOT_REACHED();
	#endif
	...
	/* F3 */
	case OFPUTIL_OFPAT11_OFX_PUSH_HEADER:
		ofx_push_header_from_openflow(&a->ofx_push_header, out);
        break;

    case OFPUTIL_OFPAT11_OFX_POP_HEADER:
        ofpact_put_OFX_POP_HEADER(out);
        break;

	case OFPUTIL_OFPAT11_OFX_PUSH_AP:
		ofpact_put_OFX_PUSH_AP(out)->action_pointer = ntohs(a->ofx_push_ap.action_pointer);
		break;

	case OFPUTIL_OFPAT11_OFX_POP_AP:
		ofpact_put_OFX_POP_AP(out);
		break;

	case OFPUTIL_OFPAT11_OFX_SET_TTL:
		ofpact_put_OFX_SET_TTL(out)->ttl = a->ofx_set_ttl.ttl;
		break;

	case OFPUTIL_OFPAT11_OFX_DEC_TTL:
		ofpact_put_OFX_DEC_TTL(out);
		break;

	case OFPUTIL_OFPAT11_OFX_JUMP:
		ofpact_put_OFX_JUMP(out)->action_pointer = ntohs(a->ofx_jump.action_pointer);
		break;	
	/* F3 */
...
ofpact_is_set_action(const struct ofpact *a)
{
    ...
    default:
    	#ifdef DEBUG
        {FILE *f1=fopen("/var/log/ovs_not_reached","ab+");fprintf(f1,"lib/ofp-actions.c 6");fflush(f1);fclose(f1);OVS_NOT_REACHED();}
	#else
	OVS_NOT_REACHED();
	#endif
    }
...
ofpact_is_allowed_in_actions_set(const struct ofpact *a)
{
    ...
    default:
    	#ifdef DEBUG
        {FILE *f1=fopen("/var/log/ovs_not_reached","ab+");fprintf(f1,"lib/ofp-actions.c 7");fflush(f1);fclose(f1);OVS_NOT_REACHED();}
	#else 
	OVS_NOT_REACHED();
	#endif
    }
...
//F3
    ...
    int i,j;
    void *data;
    FILE *fp = fopen("/var/log/ovs_decode_instructions.log","a+");
    #endif
//F3
...
//F3
    	#ifdef DEBUG
        fprintf(fp, "in processing module\n");
	fflush(fp);
	#endif
//F3
...
//F3
    	#ifdef DEBUG
        fprintf(fp, "in the left mode, left length is %d\n", left);
	fflush(fp);
        fclose(fp);
	#endif
//F3
                                   ...
                                   struct ofpbuf *ofpacts)
{
    ...
    //F3
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_pull_instruction.log","ab+");
    #endif
    //F3
    ...
    error = decode_openflow11_instructions(
        ...
        insts);
    #ifdef DEBUG
    fprintf(fp,"decode is done!error is %d\n", error);
    fflush(fp);
    #endif
    if (error) {
        goto exit;
    ...
    if (insts[OVSINST_OFPIT11_APPLY_ACTIONS]) {
    	...
    	#ifdef DEBUG
        fprintf(fp,"enter the get actions module! 1\n");
        fflush(fp);
	#endif
        get_actions_from_instruction(insts[OVSINST_OFPIT11_APPLY_ACTIONS],
                                     &actions, &max_actions);
    	#ifdef DEBUG
        fprintf(fp,"enter the get actions module! 2\n");
        fflush(fp);
	#endif
        error = ofpacts_from_openflow(actions, max_actions, version, ofpacts);
    	#ifdef DEBUG
        fprintf(fp,"enter the get actions module! 3\n");
        fflush(fp);
	#endif
        if (error) {
            goto exit;
               ...
               uint8_t table_id, uint8_t n_tables)
{
    ...
    default:
    	#ifdef DEBUG
        {FILE *f1=fopen("/var/log/ovs_not_reached","ab+");fprintf(f1,"lib/ofp-actions.c 8");fflush(f1);fclose(f1);OVS_NOT_REACHED();}
	#else
	OVS_NOT_REACHED();
	#endif
    }
...
ofpact_to_nxast(const struct ofpact *a, struct ofpbuf *out)
{
    ...
    case OFPACT_OFX_JUMP:
    	#ifdef DEBUG
        {FILE *f1=fopen("/var/log/ovs_not_reached","ab+");fprintf(f1,"lib/ofp-actions.c 9");fflush(f1);fclose(f1);OVS_NOT_REACHED();}
	#else
	OVS_NOT_REACHED();
	#endif
    }
...
ofpact_to_openflow11(const struct ofpact *a, struct ofpbuf *out)
{
    ...
    case OFPACT_METER:
    	#ifdef DEBUG
        {FILE *f1=fopen("/var/log/ovs_not_reached","ab+");fprintf(f1,"lib/ofp-actions.c 10");fflush(f1);fclose(f1);OVS_NOT_REACHED();}
	#else
	OVS_NOT_REACHED();
	#endif
    ...
    case OFPACT_OFX_PUSH_HEADER:
        //TODO
        ofx_push_header_from_openflow(ofpact_get_OFX_PUSH_HEADER(a),out);
    case OFPACT_OFX_POP_HEADER:
	ofputil_put_OFPAT11_OFX_POP_HEADER(out);
    case OFPACT_OFX_PUSH_AP:
	ofputil_put_OFPAT11_OFX_PUSH_AP(out)->action_pointer = htons(ofpact_get_OFX_PUSH_AP(a)->action_pointer);
    case OFPACT_OFX_POP_AP:
	ofputil_put_OFPAT11_OFX_POP_AP(out);
    case OFPACT_OFX_SET_TTL:
	ofputil_put_OFPAT11_OFX_SET_TTL(out)->ttl = ofpact_get_OFX_SET_TTL(a)->ttl; 
    case OFPACT_OFX_DEC_TTL:
	ofputil_put_OFPAT11_OFX_DEC_TTL(out);
    case OFPACT_OFX_JUMP:
	ofputil_put_OFPAT11_OFX_JUMP(out)->action_pointer = htons(ofpact_get_OFX_JUMP(a)->action_pointer);
    //F3
    case OFPACT_CONTROLLER:
    case OFPACT_OUTPUT_REG:
    case OFPACT_BUNDLE:
...
ofpact_put(struct ofpbuf *ofpacts, enum ofpact_type type, size_t len)
{
    ...
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_ofpact_put.log","ab+");
    fprintf(fp,"in the function! type is %d, length is %d\n", type, len);
    fflush(fp); 
    #endif

    ofpact_pad(ofpacts);
    #ifdef DEBUG
    fprintf(fp,"in the function 2\n");
    fflush(fp); 
    #endif
    ofpact = ofpacts->l2 = ofpbuf_put_uninit(ofpacts, len);
    #ifdef DEBUG
    fprintf(fp,"in the function 3\n");
    fflush(fp); 
    #endif
    ofpact_init(ofpact, type, len);
    #ifdef DEBUG
    fprintf(fp,"in the function 4\n");
    fflush(fp); 
    fclose(fp);
    #endif
    return ofpact;
