                   ...
                   const char *condition)
{
    ...
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_assert_failure.log","ab+");
    fprintf(fp, "in assert failure!\n");
    fflush(fp);
    fclose(fp);
    #endif
...
{
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_out_of_memory.log","ab+");
    fprintf(fp, "out of memoery!\n");
    fflush(fp);
    fclose(fp);
    #endif
...
ovs_abort_valist(int err_no, const char *format, va_list args)
{
    //F3
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_abort_valist.log","ab+");
    //F3
    fprintf(fp, "in valist!\n");
    fflush(fp);
    fclose(fp);
    #endif
