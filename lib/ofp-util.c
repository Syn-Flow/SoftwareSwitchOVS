...
enum ofperr
ofputil_pull_ofx_match(struct ofpbuf *buf, struct match *match,
                         uint16_t *padded_match_len)
{
    struct ofp11_match_header *omh = buf->data;
    uint16_t match_len;
...
ofputil_match_typical_len(enum ofputil_protocol protocol)
{
    ...
    default:
        {FILE *f1=fopen("/var/log/ovs_not_reached","ab+");fprintf(f1,"lib/ofp-util.c");fflush(f1);fclose(f1);OVS_NOT_REACHED();}
    }
}
                          ...
                          struct ofpbuf *ofpacts)
{
    ...
    } else if (raw == OFPRAW_OFPT11_OFX_PACKET_OUT) {
        enum ofperr error;
        const struct ofp11_packet_out *opo = ofpbuf_pull(&b, sizeof *opo);
