...
dpif_linux_flow_put(struct dpif *dpif_, const struct dpif_flow_put *put)
{
    ...
    int error;
    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs_dpif_linux_flow_put.log", "ab+");
    #endif

    dpif_linux_init_flow_put(dpif_, put, &request);
    #ifdef DEBUG
    fprintf(fp,"the key is %d, the length is %d\n", *(request.key), request.key_len);
    fflush(fp);
    #endif
...
static void
dpif_linux_F3_encode_execute(int dp_ifindex, const struct dpif_execute *d_exec,
                          struct ofpbuf *buf)
{
    struct ovs_header *k_exec;
    ...
    nl_msg_put_genlmsghdr(buf, 0, ovs_packet_family, NLM_F_REQUEST,
                          OVS_F3_PACKET_CMD_EXECUTE, OVS_PACKET_VERSION);
                 ...
                 int *dp_ifindex)
{
    ...
    type = (genl->cmd == OVS_PACKET_CMD_MISS ? DPIF_UC_MISS
            ...
            : -1);
    */
    if(genl->cmd == OVS_PACKET_CMD_MISS) {
        type = DPIF_UC_MISS;
    } else if(genl->cmd == OVS_F3_PACKET_CMD_MISS) {
        type = DPIF_UC_OFX_MISS;
    } else if(genl->cmd == OVS_PACKET_CMD_ACTION) {
        type = DPIF_UC_ACTION;
    } else if(genl->cmd == OVS_F3_PACKET_CMD_ACTION) {
        type = DPIF_UC_OFX_ACTION;
    } else {
        type = -1;
    }
...
int dpif_linux_vport_transact_debug(const struct dpif_linux_vport *request, struct dpif_linux_vport *reply, struct ofpbuf **bufp)
{
	int error;
	...
	int i,j;
	void *data;
	FILE *fp = fopen("/var/log/buf.log","ab+");
	fprintf(fp, "request_buf->size = %d\n", request_buf->size);
	data = request_buf->data;
	for(i = 0; i < request_buf->size;i++)
	{
		tmp = *((uint8_t *)data);
		data = data + 1;
		for(j = 0; j < 8; j++)
	...
	fclose(fp);
    	#endif
        error = dpif_linux_vport_transact(request, reply, bufp);
	return error;
}
...
/* F3 */
// Send user specialized message to kernel 
int 
dpif_send_ofx_port_msg(const struct dpif *dpif_, struct port_classifier *pc, const struct netdev *netdev)
{
    struct dpif_linux_vport vport_request, reply;
    int error;
    struct dpif_linux *dpif = dpif_linux_cast(dpif_);
    uint64_t options_stub[64 / 8];
    struct ofpbuf options;
    ...
    ovs_mutex_lock(&dpif->upcall_lock);
    dpif_linux_vport_init(&vport_request);
    vport_request.type = OVS_VPORT_TYPE_NETDEV;
    vport_request.cmd = OVS_VPORT_CMD_SET;
    vport_request.dp_ifindex = pc->dp_ifindex;
    vport_request.port_no = pc->port_no;
    vport_request.upcall_pid = &(pc->upcall_pid);

	    ofpbuf_use_stack(&options, options_stub, sizeof options_stub);
	    nl_msg_put_u32(&options, OVS_INFORMATION_ATTR_PORT,
			    pc->port_information);
	    vport_request.options = options.data;
	    vport_request.options_len = options.size;

    #ifdef DEBUG
    FILE *fp = fopen("/var/log/ovs.log","ab+");
    fprintf(fp, "dp_ifindex = %d port_no = %d upcall_pid = %d dpif->epoll_fd = %d options.size = %d port_information = %u\n", pc->dp_ifindex, pc->port_no, pc->upcall_pid, dpif->epoll_fd, options.size, pc->port_information);
    fclose(fp);
    #endif

    error = dpif_linux_vport_transact_debug(&vport_request, NULL, NULL);
    //pc->port_no = reply.port_no;
    //ofpbuf_delete(buf);
    ovs_mutex_unlock(&dpif->upcall_lock);
    return error;
}
...
int 
dpif_send_ofx_packet_out(int dp_ifindex, struct ofpbuf *payload)
{
    uint64_t request_stub[1024 / 8];
    struct dpif_execute execute;
    struct ofpbuf request;
    ...
    execute.packet = payload;
    execute.needs_help = 0;
    ofpbuf_use_stub(&request, request_stub, sizeof request_stub);
    dpif_linux_F3_encode_execute(dp_ifindex, &execute, &request);
    error = nl_transact(NETLINK_GENERIC, &request, NULL);
    ofpbuf_uninit(&request);
...
int 
dpif_linux_get_dpifindex(const struct dpif *dpif)
{
    struct dpif_linux *dpif_linux = dpif_linux_cast(dpif); 
    return dpif_linux->dp_ifindex;
}
int 
dpif_linux_get_upcallpid(const struct dpif *dpif_, odp_port_t odp_port, uint32_t *upcall_pid)
{
    struct dpif_linux *dpif = dpif_linux_cast(dpif_);
    uint32_t port_no = odp_to_u32(odp_port);
    struct nl_sock *sock = (port_no < dpif->uc_array_size
                            ? dpif->channels[port_no].sock
                            : NULL);
    if(!sock)
